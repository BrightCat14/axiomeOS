...
LOADER_EFI="$BUILD_DIR/bootloader/BOOTX64.EFI"
...
if [ ! -f "$LOADER_EFI" ]; then
    echo "ERROR: $LOADER_EFI not found. Run 'make bootloader' first."
    exit 1
fi
...
echo ""
echo "Step 5: Copying kernel to the ESP root (\\kernel.elf)..."
cp "$KERNEL_ELF" "$MNT/kernel.elf"

echo ""
echo "Step 6: Installing axboot UEFI bootloader..."
mkdir -p "$MNT/EFI/BOOT"
cp "$LOADER_EFI" "$MNT/EFI/BOOT/BOOTX64.EFI"
echo "  BOOTX64.EFI installed."

echo ""
echo "Step 7: (no bootloader config needed - axboot has no grub.cfg)"
...
echo "Make sure Secure Boot is disabled (axboot is unsigned)."
